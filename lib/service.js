...
Service.prototype.validate = function *() {
	if(typeof this.args !== 'object') throw new TypeError('Invalid Args');
	if(typeof this.tag !== 'string') throw new TypeError('Invalid Tagname');
	if(typeof this.name !== 'string') throw new TypeError('Invalid Name');
	if(typeof this.env !== 'object') throw new TypeError('Invalid Environment');
	...
	this.createMissingMounts();

	var deps = yield this.missingDeps();
	if(deps.length > 0) throw new Error('Missing dependencies', deps);
};

Service.prototype.createMissingMounts = function *() {
	_(this.mounts).each(function(loc, name) {
		var mountPath = path.join(this.directory, 'mounts', name);
...
Service.prototype.getConfigFiles = function *() {
	...
	return _(yield any(path.join(this.directory, 'config'))).flattenDeep().map(function(config) {
...
Service.prototype.getConfigFolders = function() {
	...
	var folders = _.flattenDeep(step(this.configs, ''));
...
Service.prototype.run = function *(nodaemon) {
	...
	var env = _(this.env).map(function(val, name) {
		return '-e ' + name + '="' + val + '"';
	}.bind(this)).value();
	...
	yield docker(args.concat(env).concat(mounts).concat(timezone).concat(configs).concat(deps).concat(ports).concat([this.tag]).concat(this.args));
};
